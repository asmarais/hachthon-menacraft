...
class Pipeline:
    ...
    async def run(self, input_type: str, file: Optional[UploadFile] = None, url: Optional[str] = None, claim_text: Optional[str] = None) -> dict:
        ...
        try:
            ...
            auth_task = self.auth.evaluate(features.copy())
            cred_task = asyncio.to_thread(self.cred.evaluate, features.copy())
            consistency_task = asyncio.to_thread(self.consistency.evaluate, features.copy())
            ...
            # Weighted: 50% Authenticity risk, 25% Credibility risk, 25% Consistency risk.
            # Consistency returns 1.0 for highly consistent, so risk is 1 - const_score.
            combined_score = (auth_score * 0.50) + ((1.0 - cred_score) * 0.25) + ((1.0 - const_score) * 0.25)
