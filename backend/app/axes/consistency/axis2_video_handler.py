...
def load_audio_from_video(video_path: str, sr: int = 16000) -> np.ndarray:
    """
    Decodes audio from video directly into a 16kHz mono float32 numpy array
    using PyAV, bypassing any dependency on an external 'ffmpeg' binary.
    """
    logger.info(f"Loading audio from {video_path} using PyAV (memory-based)")
    
    try:
        container = av.open(video_path)
        audio_stream = next((s for s in container.streams if s.type == 'audio'), None)
        if not audio_stream:
            raise RuntimeError("No audio stream found in video container.")

        # Whisper standard: 16kHz Mono float32
        # s16 is safe and stable across platforms
        ...
        for frame in container.decode(audio_stream):
            ...
            for resampled_frame in resampled_list:
                audio_frames.append(resampled_frame.to_ndarray().flatten().astype(np.float32) / 32768.0)
        ...
        logger.error(f"PyAV audio decoding failed: {str(e)}")
...
def transcribe_video(
    ...
    try:
        # Load audio data directly into memory to avoid whisper's internal ffmpeg subprocess calls
        audio_array = load_audio_from_video(video_path)
        ...
        # Passing the numpy array directly to whisper.transcribe
