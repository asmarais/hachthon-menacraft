...
class Consistency:
    ...
    def evaluate(self, features: dict[str, Any]) -> dict[str, Any]:
        ...
        handlers = {
            ...
            "text": self._document,  # Handle text alias used by other handlers
            "url": self._url,
        }
        try:
            handler = handlers.get(input_type)
            if not handler:
                # If it's something like 'account' or 'unknown', fallback gracefully
                if input_type == "account":
                    return self._url(features) # Pivot to URL/Context check
    ...
    def _document(self, f: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            # Handle direct upload documents that only have clean_text/body_text
            doc_feats = f.get("document_features")
            if not doc_feats:
                # Create a minimal context if missing, using the extracted text
                body = f.get("body_text") or f.get("clean_text") or ""
                doc_feats = {
                    "context_features": {
                        "clean_text": body,
                        "keywords": [] ,
                        "entities": [],
                        "document_embedding": [0.0] * 384 # Placeholder for missing embedding
                    }
                }
            ...
            doc_result = asyncio.run(
                analyze_context_document(
                    ...
                    claim_text=f.get("claim_text", "") or f.get("claim", ""),
